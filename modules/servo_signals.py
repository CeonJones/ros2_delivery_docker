...
class MultisineMAVController:
    ...
    def __init__(self):
        ...
        self.precision_delivery_share: str = "/develop_ws/precision_delivery_share"  # adjust if needed
        ...
        self.cmd_rate_hz: float = 20.0    # rate limit (should usually match 1/time_step)
    ...
    def _setup_mavlink(self):
        print(f"[MAVLINK] Connecting to {self.mav_connection_string} @ {self.baudrate}")
        self.master = mavutil.mavlink_connection(self.mav_connection_string, baud=115200)
    ...
    def _build_maneuver(self) -> Dict[str, np.ndarray]:
        ...
        if self.use_csv:
            ...
            if os.path.exists(csv_path):
                ...
                if csv_files:
                    latest_file = max(
                        csv_files,
                        key=lambda f: os.path.getmtime(os.path.join(csv_path, f))
                    )
                    filepath = os.path.join(csv_path, latest_file)
                    print(f"[MANEUVER] Loading most recent input signal from: {filepath}")
                    ...
                    print(f"[MANEUVER] CSV contains {len(time_vec)} samples with {num_channels} channels")
                    ...
                    print("[MANEUVER] No matching input_signal*.csv files found in Input_Data.")
            else:
            ...
            signals_dir = os.path.join(self.precision_delivery_share, "data", "signals")
            ...
            if os.path.exists(signals_dir):
                ...
                if csv_files:
                    ...
                    print("[MANEUVER] No matching input_signals*.csv files found in precision_delivery_share.")
            else:
                print(f"[MANEUVER] precision_delivery_share not found: {signals_dir}")
        ...
        print(f"Time Step: {time_step:.4f} s ({1.0 / time_step:.1f} Hz sample rate)")
    ...
    def run(self):
        ...
        print(f"[RUN] Starting maneuver with dt={dt:.4f}s, loops={self.max_loops}")
    ...
    def _post_maneuver_tmux_actions(self):
        print("[TMUX] Maneuver complete — attempting to stop rosbag and run next script via tmux.")
